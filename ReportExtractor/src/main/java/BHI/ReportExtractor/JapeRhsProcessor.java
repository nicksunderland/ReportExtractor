...
public class JapeRhsProcessor {
	...
	public JapeRhsProcessor(String examination_type,
		...
		// Get the annotations
		Annotation value_annot    = bindings.get("value")==null ? null : bindings.get("value").iterator().next();
		Annotation unit_annot     = bindings.get("unit")==null ? null : bindings.get("unit").iterator().next();
		Annotation modifier_annot = bindings.get("modifier")==null ? null : bindings.get("modifier").iterator().next();
		
		// Get the associate result from each annotation
...
//		}
		...
		String annot_value_type = value_annot.getType(); // this should work as there should be just one annotation
